...
- **`nanny_analyze.py`** — hourly oneshot (`nursery-nanny-analyze.timer`, :05 past 11:00–18:00, `Persistent=true`): 1 fps downsample → Gemini Files API upload → structured-JSON analysis (activities / phone_use with baby-context / notable_events / summary, `media_resolution=LOW` ≈ 240k tokens per camera-hour) → evidence clips for medium/high-confidence phone events cut from the raw segment → raw deleted. Chunk JSON existence in `nanny/chunks/<date>/` is the idempotency marker; failures leave raw for the next hourly retry (exit 0 — a red unit would mask config errors); >24 h-old raw is purged as a coverage gap. flock in `nanny_common.AnalyzeLock` serializes with the report's straggler sweep.
- **`nanny_report.py`** — daily oneshot (18:45, `Persistent=true`): re-runs `analyze_pending()`, then merges every chunk-date lacking a report (not "today" — Persistent catch-up after downtime merges yesterday correctly). Phone minutes are the interval **union** across cameras (double coverage counts once), split against nap windows from `storage.get_sleep_sessions_range()`. Writes `nanny/reports/<date>.json`; prunes clips past `NANNY_CLIP_RETENTION_DAYS` (14).
- **Config/secrets** live ONLY in `/etc/nursery-tracker/nanny.env` (chmod 600; seeded by `install.sh`): `GEMINI_API_KEY`, `GEMINI_MODEL` (default `gemini-2.5-flash-lite`), `NANNY_CAM_N=name=rtsp://…/stream2` (one var per camera; use the sub-stream), `NANNY_WINDOW`, `NANNY_DAYS`, `NANNY_CLIP_RETENTION_DAYS`. Cost ≈ $0.60/day for 3 cameras on flash-lite-class pricing.
