...
- **Clips show the moment, not the interval** (2026-07-31): a 4-minute phone event used to yield a 4.5-minute clip that was almost entirely static, so the evidence went unwatched — the reviewer's attention is the scarce resource, not disk. `phone_use` now carries a `key_moment` (the model names the single MM:SS that best *shows* what happened, told in the prompt that only ~20 s either side will be seen), validated in `build_chunk()` to lie inside its own event and otherwise falling back to the event start — the pickup itself is the informative part. `extract_clip(..., focus=)` cuts `KEY_MOMENT_PAD_S` (20 s) either side; without a focus it still falls back to the whole span under `CLIP_MAX_S`. Notable events are already an instant, so they centre on it. The player has 0.5×/1×/2×/4× buttons (`evidenceVideo()` in `nanny.html`) — 0.5× is there deliberately: deciding whether something really is a phone is a slow-down job.
