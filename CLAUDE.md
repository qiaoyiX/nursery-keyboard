...
**Tuning:** Follow the ordered plan in `docs/sleep-detection-research.md` §6 (noise floor → micro-motion → disturbance → end-to-end nap) using the per-frame `journalctl` numbers. For offline tuning against real footage: `bash record_camera.sh 120` on the Pi (needs `sudo apt install ffmpeg`; writes 10-min segments to `recordings/`), then `python3 replay_sleep.py recordings/<ts>/*.mp4` anywhere with opencv — it runs the identical pipeline and prints motion/presence percentiles per file. The monitor re-reads settings on reconnect (or restart the service).
